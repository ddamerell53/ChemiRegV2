...
saturn.client.core.ClientCore.prototype = {
	...
	,installNodeSocket: function() {
		...
		this.theSocket = new bindings.NodeSocket(io.connect(wsProtocol + "://" + window.location.hostname + ":" + window.location.port,{ forceNew : true}));
	...
	,authenticateSocket: function(user,cb) {
		...
		var wsProtocol = "ws";
		if(window.location.protocol == "https:") wsProtocol = "wss";
		var sock = io.connect(wsProtocol + "://" + window.location.hostname + ":" + window.location.port,{ forceNew : true});
